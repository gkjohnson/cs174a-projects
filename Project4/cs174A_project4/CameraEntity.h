...
public:
	CameraEntity():m_nearClip(1),m_farClip(500),m_fov(70),m_aspectRatio(2){}
	...
	//move left (relative to current angle) amount c
	void left(float c){
		vec3 r=getRotation();
	...
	vec3 getViewDir(){
		vec3 r=getRotation();
		vec4 v= RotateX(r.x)*RotateY(r.y)*RotateZ(r.z)*vec3(0,0,-1);
