...
DrawableEntity gamePane;
GLuint gamePaneFBO=0;

	GLuint tex_loc;


GLuint paneTexture=0;
GLuint  texture_id;
float gamePaneRatio=.7;
...
void callbackDisplay()
{
	...
	glBindFramebuffer(GL_FRAMEBUFFER,gamePaneFBO);

	glClearColor( .025, .1, .1, 1 ); // red background
	...
	aCamera.setAspectRatio(gamePaneRatio);
	...
	glClearColor( .05, .03, .05, 1 ); // pink background
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); //clear the draw buffer so that no ghosting occurs
	...
	//Drawing Game Pane
	mat4 m=mat4();
	m=Ortho2D(-.5*aspectRatio, .5*aspectRatio,.5,-.5);


	


	setCameraTransMatrix(m);
	setModelTransMatrix(gamePane.getTransMatrix());
...
/*
	...
	GLubyte* p=new GLubyte[512*512];

	glReadPixels(0,0,512,512,GL_RED,GL_UNSIGNED_BYTE,p);
	
	//std::cout<<(int)(p[1000])<<std::endl;
...
void callbackReshape(int width, int height){
	...
	if(aspectRatio<gamePaneRatio){
		glutReshapeWindow(height*gamePaneRatio,height);
	}
...
void callbackKeyboardUp(unsigned char key, int x, int y){// Called when a key is released. x, y is the current mouse position.
	...
	case ' ':
//	case 'f':
		if(fs) glutReshapeWindow(600*gamePaneRatio*1.1,600),glutPositionWindow(50,50);
		else glutFullScreen();
		fs=!fs;
	}
}
...
void initGlut(int& argc, char** argv)
{
	...
	window=glutCreateWindow("G Test");
	//glutFullScreen();
...
void createVertexArrays(){
	...
	gamePane=quads[1];
	gamePane.setScale(.5*gamePaneRatio,.5,1);
	//gamePane.scale(.2,.2,.2);
	gamePane.setPosition(0,0,0);
	...
	ilLoadImage("resources/texture2.png");
	ilCopyPixels(0, 0, 0, width, height, 1, IL_RGB,IL_UNSIGNED_BYTE, texture);
	//TEXTURE 2
	glGenTextures(1, &paneTexture);
	glBindTexture(GL_TEXTURE_2D, paneTexture);
	//specify the filters for magnification and minimization  and wrapping.
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT );
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT );
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR );
	...
	//Create seperate frame buffer for drawing :D

	//setting up
	glGenFramebuffers(1,&gamePaneFBO);
	glBindFramebuffer(GL_FRAMEBUFFER,gamePaneFBO);
	//adding depth buffer
	GLuint depthbuffer;
	...
	glGenTextures(1,&paneTexture);
	glBindTexture(GL_TEXTURE_2D,paneTexture);
	glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA8,1024,1024,0,GL_RGBA,GL_FLOAT,NULL);
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER,GL_COLOR_ATTACHMENT0,GL_TEXTURE_2D,paneTexture,0);
