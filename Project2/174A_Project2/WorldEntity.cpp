...
WorldEntity::WorldEntity():m_pos(vec3(0,0,0)),m_rot(vec3(0,0,0)),m_scale(vec3(0,0,0)){}
...
mat4 WorldEntity::getTransMatrix() const{ //returns the transformation matrix for the object
	...
	//tMatrix=Translate(m_pos.x,m_pos.y,m_pos.z)*quaternion(vec3(1,0,0),m_rot.x)*quaternion(vec3(0,1,0),m_rot.y)*quaternion(vec3(0,0,1),m_rot.z)*s;

	//tMatrix=Translate(m_pos.x,m_pos.y,m_pos.z)*quarternion(vec3(0,0,1),m_rot.z);


	return tMatrix;
}

mat4 quaternion(vec3 axis, GLfloat theta){ //creates a quaternion matrix that may or may not work properly
	...
	return 
		...
		vec4(2*x*y+2*w*z, 1-2*x*x-2*z*z, 2*y*z+w*x, 0),
		vec4(2*x*z-2*w*y, 2*y*z-w*x, 1-2*x*x-2*y*y, 0),
		vec4(0, 0, 0, 1)
		);
